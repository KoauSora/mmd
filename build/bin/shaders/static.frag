...
void main() {
    ...
    if (uUseToonTex != 0) {
        const float bias = 0.05;   // >0 brightens (pushes towards lit)
        const float scale = 1.15;  // >1 increases contrast
        float t = clamp((ndotl + bias) * scale, 0.0, 1.0);
        float v = 1.0 - t;

        // Sample both orientations to handle different toon ramp layouts.
        vec3 toonSRGB_V = texture(uToonTex, vec2(0.5, v)).rgb; // vertical ramp
        vec3 toonSRGB_H = texture(uToonTex, vec2(v, 0.5)).rgb; // horizontal ramp
